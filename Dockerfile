...
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends g++ make wget perl perl-modules
...
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends wget gnupg cpanminus libfontconfig1 && \
...
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends cpanminus make gcc libc6-dev python3 python3-pip && \
